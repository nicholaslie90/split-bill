...
- **PDF** of the whole bill, on **one long page** rather than a stack of A4s — this gets read on a phone, where scrolling is free and a page break in the middle of a card is not. Two settings, picked under the summary: **Organised** packs the figures in tight, for printing and filing; **Beautiful** gives them room, an accent rule, an inked total and a footer. The same numbers either way, and both lead with a card per person — name and total in one eyeline, the reasons beneath — because that is what the PDF gets opened for. If you scanned a struk, the photo is at the foot, behind the numbers, at full width.
...
- **Bills** at the foot of the page: every bill you have made, newest first, with what it came to. **Open** swaps to one — whatever was open goes back on the shelf on the way past, so switching never costs you anything — and **+ New bill** starts the next restaurant without touching the last one. **Save as file** writes the bill itself as `.json`, and **Open a file…** reads one back: that is how a bill moves to another phone, and the only copy of it that outlives this browser. A file is treated as untrusted like anything else — it goes through the same normalising every stored bill does.
