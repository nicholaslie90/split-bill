...
- **One drawn icon set.** Every button that carries a mark carries one from the same set — 24-unit box, two-unit stroke, round caps — so a screenful of them reads as one hand rather than a pile of found glyphs. The camera on *Scan a receipt* replaces an emoji that was whatever font the phone happened to have, and every ✕ is now drawn rather than typed. The words are still the accessible name; the mark is `aria-hidden` and only leads the eye to the right button in a column of them.
