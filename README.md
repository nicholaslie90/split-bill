...
The box under the scan button is an override, not a requirement. Paste a [Gemini API key](https://aistudio.google.com/apikey) into it and the photo goes **straight to Google on your own quota**, never through the Worker.

- The key is kept in `localStorage` on that device alone. It is never part of the bill, so emptying a bill does not clear it and no CSV, PDF or WhatsApp message carries it. **Clear** removes it and puts you back on the Worker.
- Never commit a key to this repo. It is public and GitHub Pages serves the source verbatim, so a key in the source is a key published to the world — and Google scans public repos and disables what it finds. That is why the shared key sits in a Worker secret and this box exists.
